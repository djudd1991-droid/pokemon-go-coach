...
TEXT = {'.java', '.py', '.md', '.xml', '.gradle', '.json', '.txt', '.tsv', '.properties'}
...
def main():
    ...
    parser.add_argument('--check', action='store_true', help='Verify existing archive content without writing')
    args = parser.parse_args()
    project = ROOT / 'project'
    target = ROOT / 'go-coach-source.zip'
    sources = [p for p in sorted(project.rglob('*'), key=lambda p: p.relative_to(project).as_posix())
               if p.is_file() and not any(part in EXCLUDED for part in p.relative_to(project).parts)]
    for path in sources:
    ...
    try:
        ...
        if temp.stat().st_size > LIMIT:
            raise ValueError('Source archive exceeds 25 MB; split additional assets into a pack.')
