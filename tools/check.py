...
PURE = ('Appearance LearningBank EncounterEvidence Reading PvpGrade CoachAdvice '
        'ScreenKind WildMapDetector WildMapForeground WildMapAppearance WildMapTracker RecordStore').split()
...
def main():
    ...
    checks = [('ScannerRegression', []), ('LearningCheck', []), ('AutoSaveCheck', []),
              ('MemoryCheck', [assets / 'moves.tsv']), ('RecognitionCheck', [assets / 'pvp.json']),
              ('GradeCheck', [assets / 'pvp.json']), ('AdviceCheck', [assets / 'pvp.json'])]
    output = ROOT / '.checks/classes'
    output.mkdir(parents=True, exist_ok=True)
    subprocess.run([tool('javac'), '-encoding', 'UTF-8', '-cp', str(jar), '-d', str(output),
                    *map(str, sources)], check=True)
    cp = str(output) + os.pathsep + str(jar)
    for name, arguments in checks:
        subprocess.run([tool('java'), '-cp', cp, 'com.david.gocoach.' + name,
