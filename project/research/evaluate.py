...
for p in sorted((ROOT / "references").glob("*.png")):
    ...
    kp, d = features(rgb, mask)
    refs.append((p.stem, kp, d))
        ...
        if rd is not None and desc is not None and len(desc) > 1:
            ...
            if len(good) >= 4:
                ...
                if H is not None and mask is not None:
                    chosen = dst[mask.ravel().astype(bool)]
                    inliers = len(chosen)
                    if len(chosen) >= 3:
                        spread = float(
                            cv2.contourArea(cv2.convexHull(chosen))
...
cases = [
    ("cinderace", "04-1000000532.png", (0.08, 0.10, 0.88, 0.345), "cinderace"),
    (
        "shiny-metagross",
        "03-1000000533.png",
        (0.08, 0.11, 0.88, 0.345),
        "metagross-shiny",
    ),
    (
        "decorated-lapras",
        "02-1000000534.png",
        (0.08, 0.10, 0.88, 0.345),
        "lapras-mystic",
    ),
    (
        "armored-mewtwo",
    ...
    ("mewtwo", "02-1000000542.png", (0.08, 0.10, 0.88, 0.345), "mewtwo"),
    ("glimmet-unseen", "03-1000000543.png", (0.08, 0.10, 0.88, 0.31), None),
    ("map-unrelated", "05-1000000531.png", (0.02, 0.40, 0.98, 0.79), None),
    ("bag-unrelated", "01-1000000535.png", (0.02, 0.13, 0.98, 0.44), None),
]
results = []
for label, file, box, expected in cases:
    ...
    accepted = (
        top["inliers"] >= 10
        and top["coverage"] >= 0.015
        and top["inliers"] >= runner["inliers"] + 4
    )
